...
function storageKey() {
  return `operaModelCanvas2:v${currentVersionId}`;
}
...
function defaultPosition(entity, index) {
  ...
  return {
    x: 50 + col * 330,
    y: 50 + row * 190 + Math.max(domainIndex, 0) * 24,
    collapsed: true
  };
}
...
function entityLayout(entity, index) {
  ...
  return layout[String(entity.ENTITY_ID)] || defaultPosition(entity, index);
}
...
function setupCanvasEvents() {
  ...
  stage.addEventListener('pointerup', event => {
    if (!draggingBox) return;
    updateEntityLayout(draggingBox.dataset.entityId, {
      x: parseFloat(draggingBox.style.left),
      y: parseFloat(draggingBox.style.top)
    });
  ...
  stage.addEventListener('click', event => {
    ...
    if (toggle) {
      const box = event.target.closest('.canvas2-box');
      const layout = getLayout()[String(box.dataset.entityId)] || {};
      updateEntityLayout(box.dataset.entityId, { collapsed: layout.collapsed === false });
      renderCanvas();
      return;
    }
