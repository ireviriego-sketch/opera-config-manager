(function () {
    ...
    body.innerHTML = rows.map(u => {
      ...
      return `
        <tr>
          ...
          <td><button type="button" class="btn btn-secondary btn-sm" data-edit-user="${esc(userId)}">Editar acceso</button></td>
        </tr>`;
    }).join('');
  }
  ...
  function selectedRoleCodes() {
    ...
    return new Set(roles
      .filter(role => checkedIds.has(Number(pick(role, 'roleId', 'ROLE_ID'))))
      .map(role => pick(role, 'roleCode', 'ROLE_CODE')));
  }

  function hasSelectedRole(roleCode) {
    return selectedRoleCodes().has(roleCode);
  }
  ...
  function updateRoleDependentTabs() {
    ...
    const chainPanel = $('tabChains');
    const hotelPanel = $('tabHotels');

    if (chainTab) chainTab.hidden = !showChains;
    if (hotelTab) hotelTab.hidden = !showHotels;
    if (chainPanel && !showChains) chainPanel.hidden = true;
    if (hotelPanel && !showHotels) hotelPanel.hidden = true;
    ...
    if (warning) {
      warning.hidden = showChains || showHotels;
    }
  }
  ...
  function renderRoles() {
    ...
    $('rolesChecklist').querySelectorAll('input[type="checkbox"]').forEach(input => {
      input.addEventListener('change', updateRoleDependentTabs);
    });
    updateRoleDependentTabs();
  }

  function assignedIds(scopeType) {
    return new Set((userDetail.permissions || [])
      .filter(p => pick(p, 'scopeType', 'SCOPE_TYPE') === scopeType)
      .map(p => Number(scopeType === 'CHAIN' ? pick(p, 'chainId', 'CHAIN_ID') : pick(p, 'hotelId', 'HOTEL_ID'))));
  }

  function roleIdByCode(code) {
    return Number(pick(roles.find(r => pick(r, 'roleCode', 'ROLE_CODE') === code), 'roleId', 'ROLE_ID', 0));
  }

  function renderChains(filter = '') {
    const selected = assignedIds('CHAIN');
    const q = filter.trim().toLowerCase();
    $('chainsChecklist').innerHTML = chains
  ...
  function renderHotels(filter = '') {
    const selected = assignedIds('HOTEL');
    const q = filter.trim().toLowerCase();
    $('hotelsChecklist').innerHTML = hotels
      .filter(h => !q || JSON.stringify(h).toLowerCase().includes(q))
      .map(hotel => {
  ...
  function renderPermissions() {
    ...
    if (!permissions.length) {
      $('permissionsList').innerHTML = '<p>No hay permisos técnicos asignados.</p>';
      return;
    }
    $('permissionsList').innerHTML = permissions.map(p => {
      const role = pick(p, 'roleCode', 'ROLE_CODE');
      const scope = pick(p, 'scopeType', 'SCOPE_TYPE');
  ...
  async function openUser(userId) {
    ...
    await loadCatalogs();
    const payload = await requestJson(`/api/admin/users/${currentUserId}`);
    userDetail = item(payload, 'user');
  ...
  async function saveScope(scopeType, checklistId, roleCode, readOnlyId) {
    if (scopeType === 'CHAIN' && !hasSelectedRole('CHAIN_MANAGER')) {
      alert('Marca primero el rol CHAIN_MANAGER.');
      return;
    }
    if (scopeType === 'HOTEL' && !hasSelectedRole('HOTEL_MANAGER')) {
      alert('Marca primero el rol HOTEL_MANAGER.');
      return;
    }
    ...
    if (!roleId) {
      alert(`No existe el rol ${roleCode}.`);
      return;
    }
    const url = scopeType === 'CHAIN'
      ? `/api/admin/users/${currentUserId}/permissions/chains`
      : `/api/admin/users/${currentUserId}/permissions/hotels`;
    const payload = scopeType === 'CHAIN'
      ? { roleId, chainIds: ids, isReadOnly: $(readOnlyId).checked ? 'Y' : 'N' }
      : { roleId, hotelIds: ids, isReadOnly: $(readOnlyId).checked ? 'Y' : 'N' };
    await requestJson(url, { method: 'PUT', body: JSON.stringify(payload) });
    await loadUsers();
    await openUser(currentUserId);
    showToast(scopeType === 'CHAIN' ? 'Cadenas autorizadas actualizadas correctamente' : 'Hoteles autorizados actualizados correctamente');
  }

  document.addEventListener('click', event => {
    const edit = event.target.closest('[data-edit-user]');
    if (edit) openUser(edit.dataset.editUser).catch(error => alert(error.message));
  });
  ...
  $('userSearch')?.addEventListener('input', () => {
    ...
    renderUsers(q ? users.filter(u => JSON.stringify(u).toLowerCase().includes(q)) : users);
  });
  ...
  $('newUserBtn')?.addEventListener('click', () => alert('Pendiente: alta de usuario.'));

  loadUsers().catch(error => {
    console.error(error);
    body.innerHTML = `<tr><td colspan="8">No se han podido cargar los usuarios. ${esc(error.message)}</td></tr>`;
  });
})();
