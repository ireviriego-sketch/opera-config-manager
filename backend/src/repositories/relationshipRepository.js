...
async function findByNaturalKey({ versionId, sourceAttributeId, targetAttributeId }) {
  const result = await execute(
    `${RELATIONSHIP_SELECT}
      WHERE r.version_id = :versionId
        AND r.source_attribute_id = :sourceAttributeId
        AND r.target_attribute_id = :targetAttributeId`,
    {
      versionId: Number(versionId),
      sourceAttributeId: Number(sourceAttributeId),
...
async function deleteRelationship(relationshipId, updatedBy) {
  const result = await execute(
    ...
    { relationshipId: Number(relationshipId), updatedBy },
    { autoCommit: true }
  );
  return result.rowsAffected || 0;
}
